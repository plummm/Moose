...
class LLMProvider(str, Enum):
    """Supported LLM providers (OpenAI, Anthropic, Gemini only)."""
    OPENAI = "openai"
    ANTHROPIC = "anthropic"
    GEMINI = "gemini"
    ...
    def from_string(cls, provider_str: str) -> "LLMProvider":
        ...
        try:
            ...
            raise ValueError(f"Unsupported provider: {provider_str}. Supported: openai, anthropic, gemini")
...
def get_provider(model_name: str) -> LLMProvider:
    ...
    Only supports OpenAI, Anthropic, and Gemini models.
    ...
    if model_lower.startswith("gpt-") or model_lower.startswith("o1-") or "openai" in model_lower:
        ...
        raise ValueError(
            ...
            f"Supported providers: OpenAI (gpt-*, o1-*), Anthropic (claude-*), Gemini (gemini-*)"
        )
