...
class FMPMCPTools():
    ...
    async def ask_specialist(self, target: str, instruction: str, thread_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        When to use
        - Use this tool when you need another specialist's domain tools or expertise (most commonly EDGAR filings).
        - Ask for a *specific*, reproducible action. Provide exact tool name(s) and argument values when possible.

        Parameters
        - target: str
          The specialist identifier to ask. Common targets:
          - \"edgar\" (SEC filings, ownership, financing/dilution monitoring, catalysts, statements)
          - Other FMP specialists if the meeting room defines them
          (Exact available targets depend on the current meeting room setup.)

        - instruction: str
          A clear instruction to the target specialist. Include:
          - the goal (what uncertainty you need resolved),
          - required tool call(s) and exact args (preferred),
          - the expected output format (short summary + evidence).

          Good example:
            \"Get Nvidia's last quarter's financial statements.\"
            \"What are the key dilution risks with MSFT\"
        ...
        sender_id = MeetingRoom.current_sender_id() or "fmp"
